...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({ mode: "payment", customer_email: email, billing_address_collection: "auto", line_items: [{ price_data: { currency: "eur", product_data: { name: "Don de test - Les Petits Explorateurs" }, unit_amount: cents }, quantity: 1 }], metadata: { donor_name: String(name).slice(0, 120), document_type: "test_confirmation" }, success_url: `${siteUrl}/merci-don.html?session_id={CHECKOUT_SESSION_ID}`, cancel_url: `${siteUrl}/don-annule.html` });
