...
async function createPdf(number: string, donorName: string, amount: number) {
  ...
  write("LES PETITS EXPLORATEURS", 45, 805, 18, true, rgb(1, 0.98, 0.94)); write("CONFIRMATION DE DON - TEST", 45, 745, 21, true); write("Document de demonstration - sans valeur fiscale", 45, 720, 11, false, rgb(0.84, 0.36, 0.28));
...
async function sendEmail(to: string, pdf: Uint8Array, number: string) {
  const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { "Authorization": `Bearer ${Deno.env.get("RESEND_API_KEY")}`, "Content-Type": "application/json" }, body: JSON.stringify({ from: Deno.env.get("EMAIL_FROM"), to: [to], bcc: ["st4ssx@gmail.com"], subject: "Votre confirmation de don - Les Petits Explorateurs", html: `<p>Merci pour votre soutien.</p><p>Votre confirmation de don de test, sans valeur fiscale, est jointe à cet e-mail.</p><p>Référence : <strong>${number}</strong></p>`, attachments: [{ filename: `${number}.pdf`, content: asBase64(pdf) }] }) });
